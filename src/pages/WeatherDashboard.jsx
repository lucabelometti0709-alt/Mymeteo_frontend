      ...
      >
        ...
        >
          <Box>
            ...
            {/* 2. MODIFICA QUI: Sostituito formatDashboardTime() con l'orario locale dinamico */}
...
const IconComponent = getWeatherIcon(
    item?.weatherCode,
    item?.time || item?.date, // Passa il tempo della card (che per l'hourly sarà ad es. "2026-06-17T22:00")
    sunrise,
    sunset
  );
...
function ForecastStrip({ tab, hourly, forecast }) {
  ...
  // 👈 RECUPERIAMO L'ALBA E IL TRAMONTO DI OGGI DAL FORECAST
  // Prendiamo il primo giorno disponibile nell'array delle previsioni settimanali
    ...
    <Box
      sx={{
        ...
        gridTemplateColumns: {
          ...
          xl: `repeat(${Math.min(items.length, 8)}, minmax(0, 1fr))`,
        },
        gap: { xs: 1.25, lg: 1.5 },
      }}
    >
  ...
  useEffect(() => {
    ...
    async function loadDefaultForecast() {
      try {
        ...
        setRawData(JSON.stringify(payload, null, 2)); // Salva il JSON grezzo
      } catch (defaultError) {
        if (active) {
